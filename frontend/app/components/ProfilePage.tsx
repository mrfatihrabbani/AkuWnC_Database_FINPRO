...
export default function ProfilePage({ username, currentUser, onAvatarChange, onViewProfile }: ProfilePageProps) {
  ...
  const checkFollowStatus = async () => {
    try {
      const res = await fetch(`http://localhost:3001/api/graph/following/${currentUser}`);
  ...
  const handleUnfollow = async () => {
    setFollowLoading(true);
    try {
      await fetch(`http://localhost:3001/api/graph/unfollow`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ from: currentUser, to: username }),
      });
  ...
  const fetchProfile = async () => {
    try {
      const res = await fetch(`http://localhost:3001/api/users/${username}/profile`);
  ...
  const fetchReviews = async () => {
    try {
      const res = await fetch(`http://localhost:3001/api/users/${username}/reviews`);
      const data = await res.json();
      setReviews(data);
    } catch (error) {
